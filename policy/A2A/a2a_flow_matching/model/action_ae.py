...
class MLPActionEncoder(nn.Module):
    """Simple MLP encoder for short action sequences (e.g., n_obs_steps=3)."""
    ...
    def __init__(
        ...
        input_dim = seq_length * action_dim
    ...
    def forward(self, actions, deterministic=False):
        """
        Args:
            actions: (B, seq_length, action_dim)
        Returns:
            z: (B, latent_dim)
        """
        batch_size = actions.shape[0]
        x = actions.view(batch_size, -1)  # (B, seq_length * action_dim)
