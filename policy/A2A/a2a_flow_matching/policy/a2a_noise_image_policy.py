"""
Action-to-Action Flow Matching Policy with History Noise (A2A-Noise)

A variant of A2A that adds Gaussian noise to history states before encoding.
This can improve robustness by preventing overfitting to exact state trajectories.

Architecture:
    History States [s_{t-n+1}, ..., s_t] + N(0, noise_std) --encode--> history_latents (x_0)
...
class A2ANoiseImagePolicy(BaseImagePolicy):
    """
    Action-to-Action Flow Matching Policy with History Noise.
    
    Flow Matching from noisy history states to future actions, conditioned on visual observations.
    
    - Flow START: Noisy history states (past n_obs_steps proprioceptive states + Gaussian noise)
    - Flow TARGET: Future actions (next n_action_steps)
    - CONDITION: Visual observation latents
    
    The noise is added to the raw history states before encoding, not in latent space.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # A2A-Noise specific: history noise standard deviation
    ...
    def _add_history_noise(self, history_states: torch.Tensor) -> torch.Tensor:
        """
        Add Gaussian noise to history states.
        
        Args:
            history_states: (B, T, D) tensor of history states
            
        Returns:
            Noisy history states with same shape
        """
        if self.history_noise_std > 0:
            noise = torch.randn_like(history_states) * self.history_noise_std
            return history_states + noise
    ...
    def compute_loss(self, batch):
        ...
        # A2A-Noise: Add Gaussian noise to history states before encoding
        history_states_noisy = self._add_history_noise(history_states)
        history_latents = self.history_action_encoder(history_states_noisy)
        ...
        flow_loss, metrics = self.flow_matcher.compute_loss(
            ...
            start=history_latents,  # Noisy history states as flow source
        ...
        if self.decode_flow_latents:
            # Sample with noisy history states as start and obs as condition
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        # Run flow sampling: noisy history -> future, conditioned on visual obs
        action_latents_pred = self.flow_matcher.sample(
            self.flow_net,
    ...
    def get_latents_for_visualization(self, batch):
        ...
        Returns:
            history_latents: (B, latent_dim) - encoded from noisy past states
